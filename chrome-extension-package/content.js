...
const createReactFlowMindmapPopup = (mindmapData) => {
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  switch (message.type) {
    case 'sidebar-opened':
      // Sidebar was opened via extension icon - no action needed for native side panel
      sendResponse({ success: true });
      break;
      
    case 'close-sidebar':
      // Sidebar was closed - no action needed for native side panel
      sendResponse({ success: true });
      break;
    ...
    case 'check-compatibility':
      ...
      const isCompatible = window.location.hostname.includes('chatgpt.com');
...
function getConversationTreeData(deep = false) {
  try {
    ...
    // If no floatbar, try to extract from ChatGPT page structure
...
function extractTreeDataFromDOM(ulElement) {
  ...
  console.log('Extracting tree data from DOM, items count:', items.length);
  ...
  for (let item of items) {
    if (item.tagName === 'LI') {
      ...
      // Get the message ID from the __ref if available
      let messageId = item.getAttribute('data-message-id');
      if (!messageId && item.__ref) {
        const refElement = item.__ref.deref();
        if (refElement) {
          messageId = refElement.getAttribute('data-message-id');
        }
      }
      
      // Get content from the div or from the referenced element
      ...
      if (contentDiv) {
        ...
        const refElement = item.__ref.deref();
        if (refElement) {
          const userDiv = refElement.querySelector('div[data-message-author-role="user"]');
          if (userDiv) {
            content = userDiv.textContent.trim().substring(0, 100) + '...';
          }
        }
      }
